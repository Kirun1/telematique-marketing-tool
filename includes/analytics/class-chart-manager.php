...
class ProductScraper_Chart_Manager
{
    ...
    public function get_keyword_performance_data() {
        ...
        if (!empty($keywords)) {
            ...
            $labels = ['SEO Tools', 'Product Scraper', 'WordPress Analytics', 'E-commerce SEO', 'Data Import'];
            $volumes = [1200, 950, 800, 650, 500];
            $traffic_shares = [25, 18, 15, 12, 10];
        }
    ...
    public function get_competitor_analysis_data() {
        ...
        $metrics = ['Domain Authority', 'Referring Domains', 'Organic Traffic', 'Content Score', 'Social Score'];
    ...
    private function get_fallback_traffic_data($period)
    {
        ...
        switch ($period) {
            case '7d':
                return [1200, 1350, 1450, 1600, 1550, 1300, 1400];
            case '30d':
                return [28000, 29500, 31200, 30500]; // Monthly totals
            case '90d':
                return [82000, 87500, 91200]; // Quarterly totals
            default:
                return [28000, 29500, 31200, 30500];
        }
    }
