...
class ProductScraper_OnPage_Analyzer
{
	...
	public function check_keyword_usage($post_id)
	{
		...
		$keyword_count = preg_match_all($pattern, $content);
		$word_count = preg_match_all('/\p{L}+/u', $content);
	...
	public function check_headings($post_id)
	{
		...
		$levels = $matches[1] ?? array();
		$texts = $matches[2] ?? array();
		...
		foreach ($levels as $index => $level) {
			...
			$heading_structure[] = array(
				'level' => $level,
				'text' => wp_strip_all_tags($texts[$index]),
			);
		}

		/**
		 * WordPress SEO reality:
		 * If there is no <h1> in content, the post title is assumed
		 * to be rendered as H1 by the theme.
		 */
		$has_h1 = $h1_count > 0 || !empty(get_the_title($post_id));
		...
		// H1 evaluation
		if (!$has_h1) {
			$feedback .= 'No H1 detected. ';
		} elseif ($h1_count > 1) {
			$score += 3;
			$feedback .= 'Multiple H1 headings found in content. ';
		} else {
			$score += 4;
			$feedback .= 'Valid H1 structure. ';
		}

		// Heading hierarchy
		if (!empty($heading_structure)) {
			if ($proper_hierarchy) {
				$score += 4;
				$feedback .= 'Proper heading hierarchy. ';
			} else {
				$score += 2;
				$feedback .= 'Heading levels are skipped. ';
			}
		} else {
			$feedback .= 'No headings found in content. ';
		}
		...
		if ($keyword_in_headings) {
			...
			$feedback .= 'Consider adding focus keyword to headings. ';
		}
		...
		return array(
			...
			'data' => array(
				...
				'h1_in_content' => $h1_count,
				'has_effective_h1' => $has_h1,
	...
	public function check_url_structure($post_id)
	{
		...
		$is_lowercase = ($slug === strtolower($slug));

		// Focus keyword in slug (exact, sanitized)
		if (!empty($this->focus_keyword)) {
			$keyword_slug = sanitize_title($this->focus_keyword);
			$has_keyword_in_url = stripos($slug, $keyword_slug) !== false;
		}

		// Stop-word detection (slug-only, correct)
		$stop_words = array(
			'a',
			'an',
			'the',
		...
		// Slug length (Google-friendly range)
		if ($slug_length <= 60) {
			$score += 4;
			$feedback .= 'Good URL length. ';
		} else {
			$feedback .= 'URL slug is too long. Consider shortening. ';
		}

		// Keyword presence
		if ($has_keyword_in_url) {
			$score += 3;
			$feedback .= 'Focus keyword included in URL. ';
		} else {
			$feedback .= 'Add focus keyword to the URL slug. ';
		}
