...
class ProductScraper_AI_Title_Optimizer
{
    ...
    private function generate_list_title($keyword)
    {
        ...
        return str_replace(['[number]', '{keyword}'], [$number, $keyword], $template);
    }
    ...
    private function generate_number_title($keyword)
    {
        ...
        $year = date('Y') + 1;
        $template = $templates[array_rand($templates)];
        return str_replace(['[number]', '{keyword}', '[Year]'], [$number, $keyword, $year], $template);
    }
    ...
    private function generate_comparison_title($keyword)
    {
        ...
        $template = $templates[array_rand($templates)];
        return str_replace(['{keyword}', '[Alternative]'], [$keyword, $alternative], $template);
    }
    ...
    private function analyze_title_length($title)
    {
        ...
        if ($length >= 50 && $length <= 60) {
            return array('score' => 100, 'status' => 'Perfect');
        } elseif ($length >= 40 && $length <= 70) {
            return array('score' => 80, 'status' => 'Good');
        } elseif ($length < 40) {
            return array('score' => 40, 'status' => 'Too Short');
        } else {
            return array('score' => 30, 'status' => 'Too Long');
        }
    }
    ...
    {
        $power_words = array_filter($this->power_words, function ($word) use ($title) {
